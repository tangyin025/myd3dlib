	...
	D3DVertexElementSet & VertexElems)
{
	for(unsigned int face_i = 0; face_i < NumFaces; face_i++)
	{
		...
		IntersectionTests::IntersectionType result = IntersectionTests::IntersectTriangleAndFrustum(v1, v2, v3, frustum);
		if (result == IntersectionTests::IntersectionTypeInside || result == IntersectionTests::IntersectionTypeIntersect)
		{
			return true;
		}
	}
