...
namespace boost {
	namespace serialization {
		...
		inline void serialize(Archive& ar, CRect & t, const unsigned int file_version)
		{
			ar& BOOST_SERIALIZATION_NVP(t.left);
			ar& BOOST_SERIALIZATION_NVP(t.top);
			ar& BOOST_SERIALIZATION_NVP(t.right);
			ar& BOOST_SERIALIZATION_NVP(t.bottom);
		}
	}
}
...
void StaticSkin::DrawString(UIRender* ui_render, const std::wstring& text, const my::Rectangle& rect, DWORD color)
{
	DWORD alpha = color & D3DCOLOR_ARGB(255, 0, 0, 0);
	if (m_Font && !text.empty() && alpha)
	{
		DWORD outlinealpha = m_TextOutlineColor & D3DCOLOR_ARGB(255, 0, 0, 0);
		if (outlinealpha)
		{
