...
const Font::CharacterInfo & Font::LoadCharacterOutline(unsigned long character, float outlineWidth)
{
	...
	if (FT_GLYPH_FORMAT_OUTLINE != m_face->glyph->bitmap.pixel_mode)
	{
		THROW_CUSEXCEPTION("FT_GLYPH_FORMAT_OUTLINE != ft_face->glyph->bitmap.pixel_mode");
	}
	...
	{
		THROW_CUSEXCEPTION("outlineSpans.empty()");
	}
	...
	std::vector<unsigned char> pxl(rect.Width() * rect.Height());
