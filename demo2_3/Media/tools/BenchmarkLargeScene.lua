...
for i=1,300000,1 do
	...
	local act=Actor(NamedObject.MakeUniqueName("editor_act"),Vector3(math.random(act.OctAabb.min.x,act.OctAabb.max.x),
		math.random(0,100),math.random(act.OctAabb.min.x,act.OctAabb.max.x)),Quaternion.Identity(),Vector3(1,1,1),AABB(-1,1))
