...
void ApexRenderer::renderResource(const physx::apex::NxApexRenderContext& context)
{
	...
	renderResource->m_ApexEffect->SetMatrix("g_World", (my::Matrix4 &)context.local2world.getTranspose());
	renderResource->m_ApexEffect->SetMatrix("g_ViewProj", state->m_Camera->m_ViewProj);
	renderResource->m_ApexEffect->SetTexture("g_MeshTexture", state->m_CheckerTexture->m_ptr);
	renderResource->m_ApexEffect->SetMatrixArray("g_BoneMatrices", &renderResource->m_ApexBb->m_bones[0], renderResource->m_ApexBb->m_bones.size());
	renderResource->m_ApexEffect->SetVector("g_MaterialDiffuseColor", my::Vector4(1,1,1,1));
	renderResource->m_ApexEffect->SetVector("g_MaterialAmbientColor", my::Vector4(0.1f,0.1f,0.1f,0.1f));
	renderResource->m_ApexEffect->SetVector("g_LightDir", my::Vector3(-1,-1,-1).normalize());
	renderResource->m_ApexEffect->SetVector("g_LightDiffuse", my::Vector4(1,1,1,1));
	UINT uPasses = renderResource->m_ApexEffect->Begin();
	HRESULT hr;
	for(UINT p = 0; p < uPasses; p++)
	{
		renderResource->m_ApexEffect->BeginPass(p);
		...
		renderResource->m_ApexEffect->EndPass();
	}
	renderResource->m_ApexEffect->End();
}
		...
		if(semanticData.data)
		{
			for(unsigned int j = 0; j < numVertices; j++)
			{
				...
				case physx::apex::NxRenderVertexSemantic::TEXCOORD0:
					...
					m_ve.SetTexcoord(pVertices + m_stride * j, *(my::Vector2 *)(
						(unsigned char *)semanticData.data + semanticData.stride * j), 0, 0);
					break;
				case physx::apex::NxRenderVertexSemantic::TEXCOORD1:
					_ASSERT(semanticData.format == physx::apex::NxRenderDataFormat::FLOAT2);
					m_ve.SetTexcoord(pVertices + m_stride * j, *(my::Vector2 *)(
						(unsigned char *)semanticData.data + semanticData.stride * j), 0, 1);
					break;
				case physx::apex::NxRenderVertexSemantic::TEXCOORD2:
					_ASSERT(semanticData.format == physx::apex::NxRenderDataFormat::FLOAT2);
					m_ve.SetTexcoord(pVertices + m_stride * j, *(my::Vector2 *)(
						(unsigned char *)semanticData.data + semanticData.stride * j), 0, 2);
					break;
				case physx::apex::NxRenderVertexSemantic::TEXCOORD3:
					_ASSERT(semanticData.format == physx::apex::NxRenderDataFormat::FLOAT2);
					m_ve.SetTexcoord(pVertices + m_stride * j, *(my::Vector2 *)(
						(unsigned char *)semanticData.data + semanticData.stride * j), 0, 3);
					break;
				case physx::apex::NxRenderVertexSemantic::BONE_INDEX:
					switch(semanticData.format)
					{
					case physx::apex::NxRenderDataFormat::USHORT1: {
							unsigned short value = *(unsigned short *)((unsigned char *)semanticData.data + semanticData.stride * j);
							m_ve.SetBlendIndices(pVertices + m_stride * j, D3DCOLOR_ARGB(0,0,0,value), 0, 0);
						}
						break;
					default:
						_ASSERT(false);
...
{
	m_ApexEffect = Game::getSingleton().LoadEffect("shader/ApexEffect.fx");
