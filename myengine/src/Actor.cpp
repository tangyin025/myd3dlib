...
void ViewedActorMgr::CheckViewedActor(const my::OctRoot & Root, PhysXSceneContext * scene, const my::AABB & In, const my::AABB & Out)
{
	struct Callback : public my::OctNode::QueryCallback
	{
		...
		Callback(ViewedActorMgr * _mgr, PhysXSceneContext * _scene)
			: mgr(_mgr)
			, scene(_scene)
		{
		}
		void operator() (OctActor * oct_actor, const AABB & aabb, IntersectionTests::IntersectionType)
		{
			...
			ViewedActorMgr::WeakActorMap::const_iterator actor_iter = mgr->m_ViewedActors.find(actor);
			if (actor_iter != mgr->m_ViewedActors.end())
			{
				mgr->m_ViewedActors.erase(actor);
			}
	...
	Callback cb(this, scene);
	Root.QueryActor(In, &cb);
