...
bool PhysxScene::Advance(float fElapsedTime)
{
	...
	if (m_Timer.Step(m_FrameInterval * my::D3DContext::getSingleton().m_fTimeScale))
	{
...
void PhysxScene::AdvanceSync(float fElapsedTime)
{
	...
	for (; m_Timer.Step(m_FrameInterval * my::D3DContext::getSingleton().m_fTimeScale); )
	{
		m_EventPxThreadSubstep(m_FrameInterval * my::D3DContext::getSingleton().m_fTimeScale);

		m_PxScene->simulate(m_FrameInterval * my::D3DContext::getSingleton().m_fTimeScale, NULL, 0, 0, true);
...
void PhysxScene::Substep(StepperTask & completionTask)
{
	...
	m_PxScene->simulate(m_FrameInterval * my::D3DContext::getSingleton().m_fTimeScale, &completionTask, 0, 0, true);
}
