...
{
	physx::PxActorTypeSelectionFlags flags(physx::PxActorTypeSelectionFlag::eRIGID_STATIC | physx::PxActorTypeSelectionFlag::eRIGID_DYNAMIC);
	physx::PxU32 numActors = m_PxScene->getNbActors(flags);
	if (numActors > 0)
	{
		std::vector<physx::PxActor *> actorList(numActors);
		m_PxScene->getActors(flags, &actorList[0], actorList.size(), 0);
		for (physx::PxU32 i = 0; i < actorList.size(); i++)
		{
			actorList[i]->release();
		}
	}
