...
void CChildView::QueryRenderComponent(const CPoint & level_id, const my::Frustum & frustum, RenderPipeline * pipeline, unsigned int PassMask)
{
...
BOOL CChildView::PreTranslateMessage(MSG* pMsg)
{
	...
	if (bNoFurtherProcessing)
	{
		CMainFrame * pFrame = DYNAMIC_DOWNCAST(CMainFrame, AfxGetMainWnd());
		ASSERT_VALID(pFrame);
		pFrame->ResetViewedActors(m_Camera->m_Eye, boost::static_pointer_cast<my::ModelViewerCamera>(m_Camera)->m_LookAt);
		switch (pMsg->message)
		{
		case WM_LBUTTONDOWN:
		case WM_MBUTTONDOWN:
